...
export const initLandingAnimations = () => {
  ...
  const timeoutId = setTimeout(() => {
      ...
      const onPointerMove = (e: any) => { tmx = e.clientX / innerWidth - 0.5; tmy = e.clientY / innerHeight - 0.5; };
      ...
      (function(){
        ...
        function drawPetals(t){
          ...
          ctx.globalAlpha = 1;
          const petalRafId = requestAnimationFrame(drawPetals);
          cleanupFns.push(() => cancelAnimationFrame(petalRafId));
        }
        ...
        function raf(){
          ...
          const scrollRafId = requestAnimationFrame(raf);
          cleanupFns.push(() => cancelAnimationFrame(scrollRafId));
        }
        ...
        const revealLines = Array.from(document.querySelectorAll('.reveal-line')) as HTMLElement[];
        const fadeCards   = Array.from(document.querySelectorAll('.fade-card'))   as HTMLElement[];
        ...
        fadeCards.forEach((c, i) => {
          c.style.opacity   = '0';
          c.style.transform = 'translateY(40px)';
          c.style.willChange = 'transform, opacity';
          c.dataset.revealIndex = String(i);
        });

        // Single observer handles both types
        const io = new IntersectionObserver((entries) => {
          entries.forEach(entry => {
            ...
            if (el.classList.contains('reveal-line')) {
              ...
              if (inner) {
                inner.style.transition = 'transform 0.85s cubic-bezier(0.16,1,0.3,1), opacity 0.85s ease';
                inner.style.transform  = 'translateY(0)';
                inner.style.opacity    = '1';
              }
            } else {
              // fade-card: stagger by index
              const idx = parseInt(el.dataset.revealIndex || '0', 10) % 6;
          ...
          threshold: 0,
          rootMargin: '0px 0px -60px 0px'   // fire 60px before bottom of viewport
        });

        revealLines.forEach(line => io.observe(line));
        fadeCards.forEach(c => io.observe(c));
        cleanupFns.push(() => io.disconnect());

        /* Keep GSAP only for the decorative craftBg parallax (not content) */
